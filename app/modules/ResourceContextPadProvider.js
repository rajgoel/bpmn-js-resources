...
export default function ContextPadProvider(
    ...
    canvas, rules, translate, bpmnFactory) {
...
ContextPadProvider.prototype.getContextPadEntries = function(element) {
  ...
  function appendRequest(event, element) {
    if (autoPlace) {
      ...
      appendRequestStart(event, element);
    }
  }

  function appendRequestStart(event, element) {
    var shape = createRequestShape(bpmnFactory, elementFactory);

    create.start(event, shape, element);
  }

  function appendRelease(event, element) {
    if (autoPlace) {
      var shape = createReleaseShape(bpmnFactory, elementFactory);

      autoPlace.append(element, shape);
    } else {
      appendReleaseStart(event, element);
    }
  }

  function appendReleaseStart(event, element) {
    var shape = createReleaseShape(bpmnFactory, elementFactory);
  ...
  if ((!element.businessObject || !element.businessObject.type) && !popupMenu.isEmpty(element, 'bpmn-replace')) {
    // Replace menu entry
    assign(actions, {
      'replace': {
        group: 'edit',
